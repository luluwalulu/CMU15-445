...
class Trie {
 private:
  ...
  template <class T>
  auto PutReversal(std::shared_ptr<TrieNode> p, size_t index, std::string_view key, T value) const
      -> std::shared_ptr<TrieNode>;

  auto RemoveReversal(std::shared_ptr<TrieNode> p, size_t index, std::string_view key) const
      -> std::shared_ptr<TrieNode>;
