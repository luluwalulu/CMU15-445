...
auto Optimizer::OptimizeSeqScanAsIndexScan(const bustub::AbstractPlanNodeRef &plan) -> AbstractPlanNodeRef {
  ...
  if(optimized_plan->GetType() == PlanType::SeqScan) {
    ...
    if(seq_scan_plan.filter_predicate_) {
      ...
      const ColumnValueExpression *col_expr = dynamic_cast<const ColumnValueExpression*>(cmp_expr->GetChildAt(0).get());
      ...
      for(auto* info:index_info) {
        ...
        if(attrs.size() == 1 && attrs[0] == colidx) {
          return std::make_shared<IndexScanPlanNode>(seq_scan_plan.OutputSchema(), seq_scan_plan.GetTableOid(),
