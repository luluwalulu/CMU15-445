...
- 代码结构进行了整体调整
- **发现一处错误：最后一个版本之后可能没有任何版本，这种情况下就无需删除指向下一个版本的指针**
  - 该bug修复后出现结果不匹配的错误，样例认为t4应该被删除
  - 观察后发现t4，t5本应被删除，但却没有
- **发现一处错误：假如读时间戳为2，我们不必保留一个提交时间戳为1或者2的事务，只保留一个时间戳为1或者2的UndoLog就行，只要满足后者，我们就能够回退到对应的版本了**
